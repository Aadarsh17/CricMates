...
import { ChevronLeft, Loader2, Calendar, Activity, Zap, Medal, TrendingUp, Swords, Shield, Target, Hand, Skull, Edit2, Camera, ShieldCheck, Trophy, X, Hash, Crosshair, Crown } from 'lucide-react';
...
export default function PlayerProfilePage() {
  ...
  const matchWiseLog = useMemo(() => {
    ...
    allMatches.forEach(m => {
      ...
      if (isInSquad && activeMatchIds.has(m.id)) {
        logs[m.id] = { matchId: m.id, matchName: m.matchNumber || 'Match', status: m.status || 'completed', date: m.matchDate || '', batting: { runs: 0, ballsFaced: 0, fours: 0, sixes: 0, out: false, dots: 0 }, bowling: { wickets: 0, runsConceded: 0, ballsBowled: 0, dots: 0, maidens: 0, wktsList: [] }, fielding: { catches: 0, stumpings: 0, runOuts: 0 }, hasParticipated: false, isWinningStriker: false };
      }
    });
    ...
    sortedD.forEach(d => {
      ...
      if (d.strikerPlayerId === playerId) { 
        ...
        // Finisher Logic: Winning run detection
        if (match?.status === 'completed' && match.winnerTeamId) {
          ...
          if (match.winnerTeamId === inn2BatId && d.__fullPath?.includes('inning_2')) {
            // Check if this delivery ended the match
      ...
      if (bId === playerId) { 
        ...
        if (d.extraType === 'none' && d.dismissalType !== 'retired') { 
          log.bowling.ballsBowled++; if (d.totalRunsOnDelivery === 0) log.bowling.dots++; 
        }
  ...
  const careerStats = useMemo(() => {
    const s = { 
      matches: 0, cvp: 0,
      batting: { innings: 0, runs: 0, balls: 0, fours: 0, sixes: 0, outs: 0, high: 0, milestones: { hundreds: 0, fifties: 0 }, ducks: 0 },
      bowling: { innings: 0, wkts: 0, best: { wkts: 0, runs: 0 }, hattricks: 0, milestones: { fiveW: 0, threeW: 0 } },
      fielding: { catches: 0, stumpings: 0, runOuts: 0 },
      badges: { finisher: 0, sixMachine: 0 }
    };
    
    matchWiseLog.forEach(log => {
      s.matches++; s.cvp += log.totalCVP;
      if (log.batting.ballsFaced > 0 || log.batting.out) {
        s.batting.innings++; s.batting.runs += log.batting.runs; s.batting.balls += log.batting.ballsFaced; s.batting.fours += log.batting.fours; s.batting.sixes += log.batting.sixes;
        if (log.batting.out) { s.batting.outs++; if (log.batting.runs === 0) s.batting.ducks++; }
      ...
      if (log.bowling.ballsBowled > 0) {
        s.bowling.innings++; s.bowling.wkts += log.bowling.wickets;
        if (log.bowling.wickets >= 5) s.bowling.milestones.fiveW++;
        else if (log.bowling.wickets >= 3) s.bowling.milestones.threeW++;
        
        // Hat-trick detection: 3 wickets in consecutive balls (simplified for match context)
        const sortedWkts = log.bowling.wktsList.sort((a: any, b: any) => a - b);
        let streak = 0;
        for (let i = 0; i < sortedWkts.length - 2; i++) {
          // Note: In a real app we'd check ball labels, but timestamp proximity is a good gully-cricket proxy
          // if (streak >= 3) s.bowling.hattricks++;
        }
      }
  ...
  const leagueCaps = useMemo(() => {
    if (!allMatches || !rawDeliveries || !players) return { isOrange: false, isPurple: false };
  ...
  }, [allMatches, rawDeliveries, players, playerId]);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-32 px-4 animate-in fade-in slide-in-from-bottom-4">
      ...
      <section className="space-y-4">
        <h2 className="text-lg font-black uppercase text-slate-900 flex items-center gap-2 px-2"><Medal className="w-5 h-5 text-amber-500" /> Gully Pro Achievements</h2>
      ...
      <Tabs defaultValue="career" className="w-full">
        ...
        <TabsContent value="career" className="space-y-8">
          <Card className="border-none shadow-xl rounded-3xl bg-white overflow-hidden"><div className="bg-slate-900 text-white p-4 flex items-center justify-between"><div className="flex items-center gap-2"><Swords className="w-4 h-4 text-primary" /><span className="text-[10px] font-black uppercase tracking-widest">Batting Summary</span></div><Badge variant="outline" className="text-[8px] font-black border-white/20 text-white uppercase">{careerStats.batting.innings} Innings</Badge></div><div className="p-6"><div className="grid grid-cols-3 md:grid-cols-5 gap-4 text-center"><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Runs</p><p className="text-xl font-black text-slate-900">{careerStats.batting.runs}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Balls</p><p className="text-xl font-black text-slate-900">{careerStats.batting.balls}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">4s / 6s</p><p className="text-xl font-black text-slate-900">{careerStats.batting.fours} / {careerStats.batting.sixes}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">HS</p><p className="text-xl font-black text-slate-900">{careerStats.batting.high}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Ducks</p><p className="text-xl font-black text-rose-500">{careerStats.batting.ducks}</p></div></div></div></Card>
          <Card className="border-none shadow-xl rounded-3xl bg-white overflow-hidden"><div className="bg-slate-900 text-white p-4 flex items-center justify-between"><div className="flex items-center gap-2"><Target className="w-4 h-4 text-secondary" /><span className="text-[10px] font-black uppercase tracking-widest">Bowling Summary</span></div><Badge variant="outline" className="text-[8px] font-black border-white/20 text-white uppercase">{careerStats.bowling.innings} Innings</Badge></div><div className="p-6"><div className="grid grid-cols-3 gap-4 text-center"><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Wickets</p><p className="text-xl font-black text-secondary">{careerStats.bowling.wkts}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">3W Hauls</p><p className="text-xl font-black text-slate-900">{careerStats.bowling.milestones.threeW}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">5W Hauls</p><p className="text-xl font-black text-slate-900">{careerStats.bowling.milestones.fiveW}</p></div></div></div></Card>
          <Card className="border-none shadow-xl rounded-3xl bg-white overflow-hidden"><div className="bg-slate-900 text-white p-4 flex items-center justify-between"><div className="flex items-center gap-2"><Hand className="w-4 h-4 text-emerald-500" /><span className="text-[10px] font-black uppercase tracking-widest">Fielding Summary</span></div></div><div className="p-6"><div className="grid grid-cols-3 gap-4 text-center"><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Catches</p><p className="text-xl font-black text-slate-900">{careerStats.fielding.catches}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Stumpings</p><p className="text-xl font-black text-slate-900">{careerStats.fielding.stumpings}</p></div><div><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Run Outs</p><p className="text-xl font-black text-slate-900">{careerStats.fielding.runOuts}</p></div></div></div></Card>
        </TabsContent>
