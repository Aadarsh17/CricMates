...
const STORAGE_KEY = 'cricmates_street_pro_session_v4';
...
export default function NumberGame() {
  ...
  if (gameState === 'setup') {
    return (
      <div className="max-w-md mx-auto space-y-6 py-8 px-4 animate-in fade-in slide-in-from-bottom-4">
        <div className="flex items-center gap-4"><Button variant="ghost" size="icon" onClick={() => router.push('/')} className="rounded-full"><ChevronLeft className="w-6 h-6" /></Button><h1 className="text-xl font-black uppercase tracking-widest text-slate-900">Street Session</h1></div>
        <Tabs defaultValue="registry" className="w-full">
          <TabsList className="grid w-full grid-cols-2 h-12 bg-slate-100 p-1 rounded-xl mb-6"><TabsTrigger value="registry" className="font-black text-[10px] uppercase">Registry</TabsTrigger><TabsTrigger value="league" className="font-black text-[10px] uppercase">League Table</TabsTrigger></TabsList>
          <TabsContent value="registry" className="space-y-6">
            <div className="text-center space-y-2"><div className="bg-primary w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg rotate-3"><Hash className="w-8 h-8 text-white" /></div><h1 className="text-3xl font-black uppercase tracking-tight text-slate-900">Street Pro Setup</h1><p className="text-slate-500 text-sm font-medium italic">Persistent Casual Scoring</p></div>
            <Card className="border-t-8 border-t-primary shadow-xl"><CardHeader><CardTitle className="text-xs font-black uppercase tracking-widest text-slate-400">Lineup Registry</CardTitle></CardHeader>
  ...
  if (gameState === 'finished') {
    return (
      <div className="max-w-4xl mx-auto space-y-8 py-12 px-4 text-center">
        ...
        <p className="text-slate-500 font-bold uppercase tracking-widest text-xs mb-8">Session Records Updated</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-lg mx-auto">
          ...
          <Button onClick={() => resetGame(false)} variant="outline" className="h-16 font-black uppercase tracking-widest text-lg border-2 border-primary text-primary"><RotateCcw className="w-6 h-6 mr-2" /> Start Next Match</Button>
        </div>
        <div className="mt-12 space-y-6"><h2 className="text-xl font-black uppercase tracking-widest text-slate-900 flex items-center justify-center gap-2"><ListOrdered className="w-5 h-5 text-primary" /> Session Rankings</h2><CasualLeagueTable /></div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-24 px-1 md:px-4">
      <div className="flex items-center gap-4 mb-2"><Button variant="ghost" size="icon" onClick={() => { if(confirm("Discard match?")) setGameState('setup'); }} className="text-slate-400 hover:bg-slate-100 rounded-full"><ChevronLeft className="w-6 h-6" /></Button><span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Live Street Pro v4.0</span></div>
      <Tabs defaultValue="scoring" className="w-full">
        <TabsList className="grid w-full grid-cols-3 h-12 bg-slate-100 p-1 rounded-xl mb-6 shadow-sm"><TabsTrigger value="scoring" className="font-black text-[10px] uppercase">Live</TabsTrigger><TabsTrigger value="history" className="font-black text-[10px] uppercase">Edits</TabsTrigger><TabsTrigger value="league" className="font-black text-[10px] uppercase">League</TabsTrigger></TabsList>
          ...
          <Card className={cn("border-none shadow-2xl transition-all overflow-hidden", activeStriker?.batting.out ? "ring-4 ring-destructive" : "ring-4 ring-primary")}>
            <div className="bg-slate-900 text-white p-6 md:p-8 space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                <div className="space-y-1"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary flex items-center gap-2"><UserCircle className="w-3 h-3"/> Active Striker</p><h2 className="text-3xl font-black uppercase tracking-tighter">{activeStriker?.name}</h2><div className="flex gap-4"><span className="text-xl font-bold">{activeStriker?.batting.runs} <span className="text-xs opacity-50 font-medium uppercase">Runs</span></span><span className="text-xl font-bold">{activeStriker?.batting.balls} <span className="text-xs opacity-50 font-medium uppercase">Balls</span></span></div></div>
                <div className="text-left md:text-right space-y-1"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-secondary flex items-center md:justify-end gap-2"><Zap className="w-3 h-3"/> Active Bowler</p><h2 className="text-xl font-black uppercase tracking-tighter text-slate-300">{activeBowler?.name}</h2><p className="text-[10px] font-bold text-slate-500 uppercase">Over Progress: {ballsInOver}/6 Legal</p></div>
              </div>
