...
export default function TeamsPage() {
  ...
  const handleCreateTeam = () => {
    ...
    setDocumentNonBlocking(doc(db, 'teams', teamId), { id: teamId, name: newTeam.name, logoUrl: newTeam.logoUrl || defaultTeamLogo, ownerId: user.uid, matchesWon: 0, matchesLost: 0, matchesDrawn: 0, totalRunsScored: 0, totalRunsConceded: 0, totalBallsFaced: 0, totalBallsBowled: 0, totalWicketsTaken: 0, netRunRate: 0 }, { merge: true });
    setIsCreateOpen(false); setNewTeam({ name: '', logoUrl: '' }); toast({ title: "Team Registered" });
  };
  ...
  return (
    <div className="space-y-6 pb-24 px-4 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4"><div><h1 className="text-2xl md:text-3xl font-black font-headline tracking-tight text-slate-900">Franchises</h1><p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">Calculated based on current league matches</p></div><div className="flex items-center gap-2 w-full md:w-auto"><div className="bg-slate-100 p-1 rounded flex border shrink-0"><Button variant={view === 'grid' ? "secondary" : "ghost"} size="sm" onClick={() => setView('grid')} className="h-8 w-8 p-0"><LayoutGrid className="w-4 h-4"/></Button><Button variant={view === 'list' ? "secondary" : "ghost"} size="sm" onClick={() => setView('list')} className="h-8 w-8 p-0"><List className="w-4 h-4"/></Button></div>
          {isUmpire && (
          ...
          {teams?.map(team => {
            ...
            return (
              <Card key={team.id} className="hover:shadow-md transition-all group flex flex-col border shadow-sm rounded-xl overflow-hidden bg-white">
                <CardHeader className="flex flex-row items-center justify-between p-4 pb-2 space-y-0"><div className="flex items-center space-x-3"><Avatar className="w-10 h-10 border rounded-lg bg-white shadow-sm"><AvatarImage src={team.logoUrl || defaultTeamLogo} className="object-cover" /><AvatarFallback>{team.name[0]}</AvatarFallback></Avatar><div className="min-w-0"><CardTitle className="text-base font-black tracking-tight truncate max-w-[120px] uppercase">{formatTeamName(team.name)}</CardTitle><div className="flex flex-wrap gap-1 mt-0.5">{captainName && (<p className="text-[7px] text-amber-600 font-black uppercase tracking-widest flex items-center gap-0.5"><Crown className="w-1.5 h-1.5" /> {captainName}</p>)}{vcName && (<p className="text-[7px] text-slate-500 font-black uppercase tracking-widest flex items-center gap-0.5"><Shield className="w-1.5 h-1.5" /> {vcName}</p>)}{!captainName && !vcName && (<p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Franchise</p>)}</div></div></div>{isUmpire && (<Button variant="ghost" size="icon" onClick={() => { if(confirm(`Delete ${team.name}?`)) { deleteDocumentNonBlocking(doc(db, 'teams', team.id)); toast({ title: "Team Deleted" }); } }} className="h-8 w-8 text-slate-300 hover:text-destructive"><Trash2 className="w-4 h-4"/></Button>)}</CardHeader>
                <CardContent className="flex-1 flex flex-col p-4 pt-2"><div className="grid grid-cols-3 gap-2 mb-4"><div className="bg-slate-50 p-2 rounded-lg text-center border"><p className="text-[8px] font-black uppercase text-slate-400">Wins</p><p className="text-sm font-black text-secondary">{stats.wins}</p></div><div className="bg-slate-50 p-2 rounded-lg text-center border"><p className="text-[8px] font-black uppercase text-slate-400">Losses</p><p className="text-sm font-black text-destructive">{stats.losses}</p></div><div className="bg-slate-50 p-2 rounded-lg text-center border"><p className="text-[8px] font-black uppercase text-slate-400">NRR</p><p className={cn("text-sm font-black", stats.nrr >= 0 ? "text-primary" : "text-amber-600")}>{(stats.nrr || 0).toFixed(3)}</p></div></div><Collapsible open={isHistoryOpen} onOpenChange={(open) => setOpenHistories(prev => ({...prev, [team.id]: open}))} className="border rounded-lg bg-slate-50/30 overflow-hidden"><CollapsibleTrigger asChild><Button variant="ghost" className="w-full flex items-center justify-between px-3 py-2 h-auto hover:bg-slate-100"><div className="flex items-center gap-2"><History className="w-3 h-3 text-slate-400" /><span className="text-[9px] font-black text-slate-500 uppercase">Recent Form</span></div>{isHistoryOpen ? <ChevronUp className="w-3 h-3 text-slate-400" /> : <ChevronDown className="w-3 h-3 text-slate-400" />}</Button></CollapsibleTrigger><CollapsibleContent className="px-2 pb-2"><TeamMatchHistory teamId={team.id} matches={allMatches} teams={teams || []} /></CollapsibleContent></Collapsible><div className="mt-4 pt-4 border-t"><Button variant="outline" className="w-full text-[10px] font-black uppercase tracking-widest h-10 shadow-sm" asChild><Link href={`/teams/${team.id}`}>View Squad & Leadership</Link></Button></div></CardContent>
              </Card>
            );
          })}
        </div>
