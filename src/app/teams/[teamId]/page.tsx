...
export default function TeamDetailsPage() {
  ...
  const squadStats = useMemo(() => {
    ...
    players.forEach(p => { careerTotals[p.id] = { id: p.id, name: p.name, runs: 0, wickets: 0, cvp: 0, wides: 0, noBalls: 0 }; });
    allDeliveries.forEach(d => {
      const matchId = d.__fullPath?.split('/')[1];
      ...
      if (careerTotals[sId] && pMatchStats[sId]?.[matchId!]) {
        const sStats = pMatchStats[sId][matchId!]; sStats.runs += d.runsScored || 0;
        if (d.extraType !== 'wide') sStats.ballsFaced += 1; careerTotals[sId].runs += d.runsScored || 0;
      }
      if (bId && careerTotals[bId] && pMatchStats[bId]?.[matchId!]) {
        const bStats = pMatchStats[bId][matchId!]; bStats.runsConceded += d.totalRunsOnDelivery || 0;
        if (d.extraType !== 'wide' && d.extraType !== 'noball') bStats.ballsBowled += 1;
        if (d.isWicket && d.dismissalType !== 'runout' && d.dismissalType !== 'retired') { bStats.wickets += 1; careerTotals[bId].wickets += 1; }
        if (d.extraType === 'wide') careerTotals[bId].wides += 1; if (d.extraType === 'noball') careerTotals[bId].noBalls += 1;
      }
      if (fId && careerTotals[fId] && pMatchStats[fId]?.[matchId!]) {
        const fStats = pMatchStats[fId][matchId!];
        if (d.dismissalType === 'caught') fStats.catches += 1; if (d.dismissalType === 'stumped') fStats.stumpings += 1; if (d.dismissalType === 'runout') fStats.runOuts += 1;
      }
    });
    Object.keys(careerTotals).forEach(id => {
      const matchHistory = pMatchStats[id] || {}; let totalCvp = 0;
  ...
  if (!isMounted || isTeamLoading || isDeliveriesLoading || isMatchesLoading) return (<div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4"><Loader2 className="w-10 h-10 text-primary animate-spin" /><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Syncing Team History...</p></div>);
      ...
      {extrasLeaderboard.length > 0 && (
        <Card className="border-t-4 border-t-amber-500 shadow-sm">
          <CardHeader className="py-4">
            <CardTitle className="text-sm font-black uppercase tracking-widest flex items-center gap-2"><AlertCircle className="w-4 h-4 text-amber-500" /> Extras contribution</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {extrasLeaderboard.map((player) => (
              <div key={player.id} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg border">
                ...
                <span className="font-bold text-[10px] text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">{player.totalExtras} - [{player.wides || 0} WD & {player.noBalls || 0} NB]</span>
              </div>
            ))}
          </CardContent>
        </Card>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className="relative group/photo">
                      <Avatar className="w-14 h-14 rounded-xl shadow-sm border overflow-hidden"><AvatarImage src={player.imageUrl} className="object-cover" /><AvatarFallback className="font-black text-slate-400 bg-slate-50">{player.name[0]}</AvatarFallback></Avatar>
                      {user && ( <button onClick={() => openEditDialog(player)} className="absolute -bottom-1 -right-1 bg-white p-1 rounded-lg border shadow-sm text-primary hover:text-primary/70 transition-colors"><Camera className="w-3 h-3" /></button> )}
                    </div>
                  ...
                  <div className="flex gap-1">
                    {user && ( <Button variant="ghost" size="icon" onClick={() => openEditDialog(player)} className="h-8 w-8 text-slate-300 hover:text-primary"><Edit2 className="w-3.5 h-3.5" /></Button> )}
          ...
          <div className="space-y-6 py-4">
            <div className="flex flex-col items-center gap-4">
              <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                <Avatar className="w-24 h-24 border-4 border-slate-100 shadow-lg rounded-2xl overflow-hidden"><AvatarImage src={editForm.imageUrl} className="object-cover" /><AvatarFallback className="bg-primary text-white text-3xl font-black">{editForm.name?.[0] || '?'}</AvatarFallback></Avatar>
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl flex flex-col items-center justify-center text-white"><Upload className="w-6 h-6 mb-1" /><span className="text-[8px] font-black uppercase">Upload</span></div>
